...
- The Firezone server module `services.firezone.server` has been updated and now uses a unified portal architecture. This required changes to the module structure and the previous multi-component setup (domain, web, api) had to be consolidated into a single main service. Component specific settings were removed and only the global `services.firezone.settings` option remains. After transferring settings into the global option, no further changes are necessary.
